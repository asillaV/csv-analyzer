...
class CsvAnalyzer:
    ...
    def detect_delimiter_and_header(self, encoding: str) -> Dict[str, Optional[str]]:
        ...
        delimiters = [",", ";", "\t", "|"]
        # Leggo poche righe senza esplodere su caratteri strani
        ...
        def text_ratio(row: str) -> float:
            ...
            if not cells:
                return 0.0
            return sum(any(c.isalpha() for c in cell) for cell in cells) / len(cells)
        ...
        if len(lines) > 1:
            ...
            # Solo una riga: se contiene testo → header, altrimenti no
    ...
    def extract_columns(self, encoding: str, delimiter: str, header: Optional[int]) -> List[str]:
        ...
        try:
            with open(self.path, "r", encoding=encoding, errors="ignore") as f:
                if header is None:
                    # Leggi la prima riga per contare le colonne
                ...
                for i, line in enumerate(f):
                    if i == header:
                        columns = [self._clean_name(c) for c in line.strip().split(delimiter)]
                        log.info("Extracted %d columns from header row %d.", len(columns), header)
                        return columns
        except Exception as e:
            log.error("Error extracting columns: %s", e)
    ...
    def analyze(self) -> Dict:
        ...
        try:
            ...
            log.info(
                ...
                info["encoding"], info["delimiter"], info["header"]
            )
            ...
            log.error("Error analyzing file: %s", e, exc_info=True)
            raise
...
def analyze_csv(path_str: str) -> Dict:
    ...
    columns: List[str] = [CsvAnalyzer._clean_name(c) for c in result.get("columns", [])]
