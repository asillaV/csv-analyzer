...
DEFAULT_OUTDIR = ROOT_DIR / "tests_csv"
...
def format_number_locale(
    ...
    """Format a number using locale-like separators."""
    ...
    int_formatted = thousands.join(chunks) if thousands else "".join(chunks)
    if decimals == 0:
        return f"{sign}{int_formatted}"
    return f"{sign}{int_formatted}{decimal}{frac_part}"


def generate_all(outdir: Path = DEFAULT_OUTDIR) -> None:
    ensure_dir(outdir)
    rng = np.random.default_rng(1234)

    # 1️ Test base: singola colonna numerica
    df1 = pd.DataFrame({"Value [V]": np.linspace(0, 10, 50)})
    df1.to_csv(outdir / "01_basic.csv", index=False)
    ...
    df2.to_csv(outdir / "02_with_x_numeric.csv", index=False)
    ...
    df3.to_csv(outdir / "03_with_x_datetime.csv", index=False)
    ...
    df4.to_csv(outdir / "04_noise_signal.csv", index=False)
    ...
    df5.to_csv(outdir / "05_multicolumn.csv", index=False)
    ...
    df6.to_csv(outdir / "06_nan_and_inf.csv", index=False)

    # 7️ Serie troppo corta per FFT
    df7 = pd.DataFrame(
        {"Time [s]": [0.0, 0.1, 0.2], "Value [V]": [1.0, 2.0, 3.0]}
    )
    df7.to_csv(outdir / "07_short_signal.csv", index=False)
    ...
    df8.to_csv(outdir / "08_big_signal.csv", index=False)

    # 9º Locale italiano: separatore ';', decimale ',' e migliaia '.'
    n9 = 80
    t9 = np.linspace(0, 240, n9)
    pressure9 = 1.8 + 0.25 * np.sin(np.linspace(0, 6 * np.pi, n9)) + rng.normal(
        0, 0.05, n9
    )
    ...
    df9 = pd.DataFrame(
        {
            "Tempo [s]": [
                format_number_locale(v, decimals=1, decimal=",", thousands=".")
                for v in t9
            ],
            "Pressione [bar]": [
                format_number_locale(v, decimals=2, decimal=",", thousands=".")
                for v in pressure9
            ],
            "Portata [L/min]": [
                format_number_locale(v, decimals=1, decimal=",", thousands=".")
    ...
    df9.to_csv(outdir / "09_locale_it.csv", index=False, sep=";")
    ...
    df10 = pd.DataFrame(
        {
            ...
            "Energia [Wh]": [
                format_number_locale(
                    v, decimals=0, decimal=",", thousands="\u00A0"
                )
                for v in energy10
            ],
            "Temp [°C]": [
            ...
            "Umidità [%]": [
                (
                    ...
                    + "%"
                    if i % 7 == 0
                    else format_number_locale(
                        v, decimals=1, decimal=",", thousands=None
                    )
                )
    ...
    df10.to_csv(outdir / "10_tab_space_thousands.csv", index=False, sep="\t")
    ...
    df11.to_csv(outdir / "11_mixed_tokens.csv", index=False)
    ...
    margin = revenue - cost
    margin_pct = (
        np.divide(margin, revenue, out=np.zeros_like(margin), where=revenue != 0) * 100
    )
    ...
    df12.to_csv(outdir / "12_currency_euro.csv", index=False, sep=";")

    print(f"Tutti i file creati in: {outdir}")
